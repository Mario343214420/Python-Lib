...
"""
https://www.52pojie.cn/thread-1445974-1-1.html
...
for i in range(k):
    if i < k - 1:
        ...
        for i in range(awemenum):
            ...
            videotitle = data['aweme_list'][i]['desc'].replace("?", "").replace("\"", "＼").replace("\x83", "＼").replace(
                "\x08", "＼").replace("\n", "＼").replace("\r", "＼").replace(":", "：").replace("/", "／").replace("|",
                                                                                                               "｜").replace(
                ":", "：").replace("#", "＃").replace(">", "＞").replace("<", "＜").replace("@抖音小助手", "").replace(
                "@DOU+小助手", "").replace("@抖音科技", "").replace("@抖音财经", "").replace("@DOU知计划", "")
